...
Agar **har URL** par scan TLS error de (e.g. `unable to verify the first certificate`), tumhare office / antivirus **HTTPS inspect** kar raha hai.

1. **Pehle yeh try karo** — default `npm run go-live:audit` ab Node **20.6+** par system CA use karta hai (`scripts/go-live-audit-run.cjs`).

2. Phir bhi fail ho to **sirf trusted network** par:

   **PowerShell**

   ```powershell
   npm run go-live:audit:insecure-tls
   ```

   (Yeh `GO_LIVE_AUDIT_TLS_INSECURE=1` set karke server chalata hai — MITM risk samajh kar.)

3. Ya manually env:

   `GO_LIVE_AUDIT_TLS_INSECURE=1` (jahan tunnel / local server chal raha hai)
